...
QColor chevronButtonBackground( const QPalette& palette, bool isDown )
{ return WINUI3Colors[ isDarkPalette( palette ) ? 1 : 0 ][ isDown ? subtlePressedColor : subtleHighlightColor ]; }

QPainterPath roundedPanelPath( const QRectF& rect, bool roundTopLeft, bool roundTopRight, bool roundBottomRight, bool roundBottomLeft )
{
...
protected:
    void paintEvent( QPaintEvent* ) override
    {
        ...
        const QPainterPath fillPath =
            roundedPanelPath( bounds, !down && m_outerEdge, !down && m_outerEdge, down && m_outerEdge, down && m_outerEdge );
...
public:
    ContentViewport( ExExpander* expander, ContentStack* panel )
        : QWidget( expander )
        , m_expander( expander )
        , m_clipViewport( new QWidget( this ) )
        , m_panel( panel )
    {
        setMinimumWidth( ExpanderMinWidth );
        // 展开时外层一次性占用完整 Content 高度，动画只发生在内层
        // 裁剪窗口；收起时则继续逐步回收布局高度。
        setSizePolicy( QSizePolicy::Preferred, QSizePolicy::Maximum );
        panel->setParent( m_clipViewport );
    ...
    QSize sizeHint() const override
    {
        ...
        result.setHeight( layoutHeight() );
        return result;
    }

    QSize minimumSizeHint() const override { return QSize( ExpanderMinWidth, 0 ); }

protected:
    void resizeEvent( QResizeEvent* event ) override
    {
        QWidget::resizeEvent( event );
        // 滚动条出现可能改变可用宽度，因此按最终宽度重新测量一次。
        // 展开阶段的后续动画帧仍只改变内层裁剪窗口。
    ...
    {
        if ( watched == m_panel && event->type() == QEvent::LayoutRequest
             && m_expander->m_expansionAnimation->state() != QAbstractAnimation::Running )
        {
            prepareAnimation();
        }
...
private:
    int layoutHeight() const { return m_expander->isExpanded() ? m_fullHeight : qRound( m_fullHeight * m_viewportProgress ); }
    ...
    void setFullHeight( int height )
    {
        ...
        {
            // expanded 状态可能刚刚切换，完整高度未变，但布局高度的
            // 计算方式已经从动画进度切换为一次性预占（或反过来）。
            updateLayoutHeight();
            return;
        }
        m_fullHeight = height;
    ...
    void updateLayoutHeight()
    {
        ...
        updateGeometry();
        m_expander->m_rootLayout->invalidate();
    ...
    void updatePanelGeometry()
    {
        ...
        const bool up           = m_expander->expandDirection() == ExExpander::Up;
        const int clipY         = up ? qMax( 0, height() - visibleHeight ) : 0;
        const QRect clipGeometry( 0, clipY, width(), visibleHeight );
        if ( m_clipViewport->geometry() != clipGeometry )
        {
            m_clipViewport->setGeometry( clipGeometry );
        }

        // Down 从顶部向下揭示，Up 从底部向上揭示。
        const int panelY = up ? visibleHeight - m_fullHeight : 0;
        const QRect panelGeometry( 0, panelY, width(), m_fullHeight );
        if ( m_panel->geometry() != panelGeometry )
        {
            m_panel->setGeometry( panelGeometry );
        }
    }

    ExExpander* m_expander   = nullptr;
    QWidget* m_clipViewport  = nullptr;
...
public:
    ...
    QWidget* headerWidget() const { return m_headerWidget.data(); }
...
protected:
    bool event( QEvent* event ) override
    {
        ...
        if ( event->type() == QEvent::Enter || event->type() == QEvent::Leave || event->type() == QEvent::HoverEnter
             || event->type() == QEvent::HoverLeave || event->type() == QEvent::EnabledChange
    ...
    void paintEvent( QPaintEvent* ) override
    {
        ...
        const bool down      = m_expander->expandDirection() == ExExpander::Down;
        const QRectF bounds  = QRectF( rect() ).adjusted( 0.5, 0.5, -0.5, -0.5 );
        const QPainterPath headerPath =
            roundedPanelPath( bounds, !connected || down, !connected || down, !connected || !down, !connected || !down );
        ...
        const qreal chevronX = rtl ? ChevronTrailingMargin : width() - ChevronTrailingMargin - ChevronButtonSize;
        const QRectF chevronRect( chevronX, ( height() - ChevronButtonSize ) * 0.5, ChevronButtonSize, ChevronButtonSize );
        if ( isEnabled() && ( underMouse() || isDown() ) )
        {
            painter.setPen( Qt::NoPen );
            painter.setBrush( chevronButtonBackground( palette(), isDown() ) );
        ...
        painter.setFont( ExFontIcon::iconFont( 15 ) );
        const QRectF glyphRect( -ChevronButtonSize * 0.5, -ChevronButtonSize * 0.5, ChevronButtonSize, ChevronButtonSize );
        painter.drawText( glyphRect, Qt::AlignCenter, ExFontIcon::iconString( SegoeIcon::ChevronDownMed ) );
        painter.restore();
    ...
    , m_expansionAnimation( new QVariantAnimation( this ) )
{
    ...
    connect( m_expansionAnimation, &QVariantAnimation::finished, this, &ExExpander::finishTransition );
}
...
{
    if ( widget == this || widget == m_headerButton || widget == m_contentContainer || widget == m_contentStack
         || isInternalWidget( widget ) || isContentWidgetOrDescendant( widget ) || widget == m_headerWidget.data()
         || ( widget && m_headerWidget && m_headerWidget->isAncestorOf( widget ) ) || ( widget && widget->isAncestorOf( this ) ) )
    {
        return;
    }
    if ( m_headerWidget )
    ...
    if ( widget )
    {
        m_headerWidgetDestroyedConnection = connect( widget,
                                                     &QObject::destroyed,
                                                     this,
                                                     [ this ]
                                                     {
...
{
    if ( !widget || widget == this || widget == m_headerButton || widget == m_contentContainer || widget == m_contentStack
         || isInternalWidget( widget ) || widget == m_headerWidget.data()
         || ( widget && m_headerWidget && m_headerWidget->isAncestorOf( widget ) ) || isContentWidgetOrDescendant( widget )
    ...
    m_contentWidgetDestroyedConnections.append( connect( widget,
                                                         &QObject::destroyed,
                                                         this,
                                                         [ this, panel ]
                                                         {
...
{ return widget && ( m_headerButton->isAncestorOf( widget ) || m_contentContainer->isAncestorOf( widget ) ); }
...
void ExExpander::refreshContentGeometry()
{
    ...
    m_contentContainer->prepareAnimation();
    m_contentContainer->setViewportProgress( m_expansionProgress );
    const bool visible = ( m_expanded || m_expansionAnimation->state() == QAbstractAnimation::Running ) && hasContentWidgets();
...
void ExExpander::setExpanded( bool expanded )
{
    ...
    m_expansionAnimation->stop();
    const bool canAnimate = m_animationEnabled && m_animationDuration > 0 && parentWidget() && parentWidget()->isVisible();
    if ( !canAnimate )
    {
    ...
    // 展开时外层 ContentViewport 先一次性进入最终布局，后续动画只
    // 改变内部裁剪窗口；收起时仍逐步回收布局高度，完成后再隐藏。
    m_contentContainer->setVisible( hasContentWidgets() );
    m_contentStack->setVisible( hasContentWidgets() );
    m_contentContainer->prepareAnimation();
    m_rootLayout->activate();
    ...
    if ( expanded )
    {
        ...
        {
            m_expansionAnimation->setDuration( qMax( 1, qRound( duration * ( 1.0 - m_expansionProgress ) ) ) );
    ...
    {
        m_expansionAnimation->setDuration( qMax( 1, qRound( m_animationDuration * m_expansionProgress ) ) );
        m_expansionAnimation->setEasingCurve( QEasingCurve::OutCubic );
        m_expansionAnimation->setStartValue( m_expansionProgress );
...
void ExExpander::changeEvent( QEvent* event )
{
    ...
    if ( event->type() == QEvent::PaletteChange || event->type() == QEvent::ApplicationPaletteChange || event->type() == QEvent::StyleChange
         || event->type() == QEvent::LayoutDirectionChange || event->type() == QEvent::EnabledChange )
    {
        m_headerButton->update();
        for ( ContentPanel* panel : m_contentPanels )
        {
