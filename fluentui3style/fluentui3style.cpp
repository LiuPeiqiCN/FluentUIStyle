...
static void installSliderValueTipHooks( QSlider* slider )
{
    ...
    QObject::connect( slider, &QSlider::sliderPressed, slider, [ slider ]() { showSliderValueTip( slider, slider->sliderPosition() ); } );
    ...
    QObject::connect( slider, &QSlider::sliderReleased, slider, [ slider ]() { hideSliderValueTip( slider ); } );
}
...
static QColor segmentedThemeColorProperty( const QWidget* widget,
                                           bool darkTheme,
                              ...
                              QRect* iconRect )
{
    ...
    const bool isVerticalNavigation = isNavigationStyle
                                      && ( opt->shape == QTabBar::RoundedEast || opt->shape == QTabBar::RoundedWest
...
inline int getColorSchemeIndex()  // 0 = Light, 1 = Dark
{
    ...
    // 如果没有设置，则根据系统主题色返回（QStyleHints::colorScheme /
    // Qt::ColorScheme 仅 Qt 6.5+）
    ...
    : QProxyStyle( createBaseStyle( style ) )
{
    ...
    {
        qWarning() << "[FluentUI3Style] Failed to load Segoe Fluent Icons font "
                      "from resource. Falling back to system font.";
        ...
        case CC_ComboBox :
        {
            if ( const QStyleOptionComboBox* combobox = qstyleoption_cast<const QStyleOptionComboBox*>( option ) )
            {
                ...
                const QColor opaqueBg =
                    /*resolveOpaque( rawBrush.color(), blendBase )*/ rawBrush.color();
...
void FluentUI3Style::drawSpecialButton( QPainter* painter, const QStyleOption* option, const QWidget* widget, bool& isReturn ) const
{
    ...
    else if ( objectName == "win_caption_minimize" || objectName == "win_caption_maximize" || objectName == "win_caption_theme"
              || objectName == "win_caption_pin" )
    {
        isReturn = true;
        if ( isDown )
        {
        ...
        case PE_PanelLineEdit :
            if ( const auto* panel = qstyleoption_cast<const QStyleOptionFrame*>( option ) )
            {
                ...
                if ( !isInSpinBox && !isInComboBox )
                {
                    ...
                    const QColor opaqueBg = resolveOpaque( inputFillBrush( option, widget ).color(), option->palette.base().color() );
                    drawRoundedRect( painter, frameRect, Qt::NoPen, opaqueBg );
            ...
            if ( const QStyleOptionViewItem* vopt = qstyleoption_cast<const QStyleOptionViewItem*>( option ) )
            {
                ...
                if ( isNonTableHighlight )
                {
...
#else
                    if ( const QTreeView* tv = qobject_cast<const QTreeView*>( widget ) )
                    {
                        // 核心 Hack: 利用坐标系区分 QTreeView 的两次调用
                        // 第一次调用(树枝): rect
                        // 起点刚好在列的视觉起点，且宽度通常小于整列宽度
                        // 第二次调用(文本底板): rect 起点被推后了 indent 的距离
                        int treeCol     = tv->treePosition();
                        int colX        = tv->header()->sectionViewportPosition( treeCol );
                        int sectionSize = tv->header()->sectionSize( treeCol );

                        if ( isRtl )
                        {
                            isDecorationColumn = ( vopt->rect.right() + 1 == colX + sectionSize );
                        }
                        else
                        {
                            isDecorationColumn = ( vopt->rect.left() == colX );
                        }

                        // 防御性补丁：如果是顶层节点且没有缩进 (indent == 0)
                        // 此时第一次调用会被 Qt 跳过，第二次调用宽度占满全列。
                        // 我们不应该在这里画，全权交给 CE_ItemViewItem 即可。
                        if ( vopt->rect.width() >= sectionSize )
                        {
                    ...
                    if ( isDecorationColumn && vopt->showDecorationSelected )
                    {
                        ...
                        const QColor itemHighlight = isSelected ? winUI3Color( subtlePressedColor ) : winUI3Color( subtleHighlightColor );
...
#    endif
    ...
    // Fixed bar width while sliding (no stretch / pulse); min keeps bar inside
    // both tab slots if widths differ
...
void FluentUI3Style::drawSegmentedSlideTab( const QStyleOptionTab* tab, QPainter* painter, const QWidget* widget ) const
{
    ...
    const QColor pressedColor = segmentedThemeColorProperty( widget,
                                                             colorSchemeIndex == 1,
                                                             SegmentedPressedColorProperty,
                                                             SegmentedPressedColorDarkProperty,
                                                             winUI3Color( subtlePressedColor ) );
    const QColor selectedColor =
        segmentedThemeColorProperty( widget,
                                     colorSchemeIndex == 1,
...
void FluentUI3Style::drawSegmentedWinUI3Tab( const QStyleOptionTab* tab, QPainter* painter, const QWidget* widget ) const
{
    ...
    if ( selectProgress > 0.0 || isSelected )
    {
        ...
        {
            QRectF indicatorRect = QRectF( adjustedTabRect.x() + ( adjustedTabRect.width() / 2.0 - currentIndicatorWidth / 2.0 ),
                                           adjustedTabRect.bottom() - 3.5,
...
void FluentUI3Style::drawNavigationViewIndicator( const QStyleOptionViewItem* option, QPainter* painter, const QWidget* widget ) const
{
    ...
    if ( !inAnimation && phase.isEmpty() )
    {
        ...
        if ( wasSelected && !hasSelection && toIndex.isValid() && toH > 0.0 )
        {
            ...
            fromH     = toH;
            fromIndex = toIndex;
            toTop     = ( navDir == QStringLiteral( "up" ) ) ? ( fromTop - normalInset ) : ( fromTop + fromH + normalInset );
            toH       = 0.0;
...
// From QCommonStyle::drawControl, but only for CE_TabBarTabLabel, and with some
// adjustments for FluentUI3
void FluentUI3Style::drawTabBarTabLabel( const QStyleOption* option, QPainter* painter, const QWidget* widget ) const
{
    if ( const QStyleOptionTab* tab = qstyleoption_cast<const QStyleOptionTab*>( option ) )
    {
        ...
        {
            // compute tr again, unless tab is moving, because the style may override
            // subElementRect
#if QT_VERSION >= QT_VERSION_CHECK( 6, 0, 0 )
        ...
        else if ( !tab->icon.isNull() && tab->text.isEmpty() )
        {
            ...
            iconRect        = QRect( tab->rect.center().x() - iconW / 2, tab->rect.center().y() - iconH / 2, iconW, iconH );
            tr              = tab->rect;
            alignment       = Qt::AlignCenter | Qt::TextShowMnemonic;
        }
        ...
        QColor tabTextColor;
        const bool isSegmentedCustomColors = widget
    ...
    switch ( element )
    {
        ...
        case CE_ToolButtonLabel :
            ...
            if ( const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>( option ) )
            {
                ...
                bool hasArrow =
                    /*toolbutton->features & QStyleOptionToolButton::Arrow*/ false;
                ...
                else
                {
                    ...
                    if ( !toolbutton->icon.isNull() )
                    {
                        ...
                        // Note: 由于 Qt 源码中 QDockWidgetTitleButton::dockButtonIconSize()
                        // 有硬编码限制， 即使在 FluentUI3Style 中设置 PM_SmallIconSize 为
                        // 20，也会被 isWindowsStyle 检查限制为 10 像素左右。 这里直接硬编码为
                        // 15 像素，以获得更大的 dock widget title 按钮图标。
...
#    else
                        ...
                        pm                = toolbutton->icon.pixmap(
                            qt_getWindow( widget ), toolbutton->rect.size().boundedTo( tBtnIconSize ), mode, state );
#    endif
                    ...
                    if ( toolbutton->toolButtonStyle != Qt::ToolButtonIconOnly )
                    {
                        ...
                        {
                            pr.setHeight( pmSize.height() + 4 );  // ### 4 is currently hardcoded
        ...
        case CE_ProgressBarContents :
            if ( const auto baropt = qstyleoption_cast<const QStyleOptionProgressBar*>( option ) )
            {
                ...
                if ( rect.width() > 1.0 )
                {
                    // After the vertical-path transform, width is still along progress and
                    // height is thickness. Cap insets belong on the progress axis only; do
                    // not shrink thickness for vertical bars.
        ...
        case CE_PushButtonBevel :
            if ( const QStyleOptionButton* btn = qstyleoption_cast<const QStyleOptionButton*>( option ) )
            {
                ...
                if ( btn->features.testFlag( QStyleOptionButton::HasMenu ) )
                {
                    ...
                    const auto indRect   = QRect( rect.right() - indSize - 2, rect.top(), indSize, rect.height() );
                    const auto vindRect =
                        /*visualRect(btn->direction, textRect, indRect)*/ indRect;
        ...
        case CE_MenuItem :
            ...
            if ( const auto* menuitem = qstyleoption_cast<const QStyleOptionMenuItem*>( option ) )
            {
                ...
                if ( act && dis == false )
                {
                    ...
                    QBrush selectColor                 = highContrastTheme ? menuitem->palette.brush( QPalette::Highlight )
...
#    endif
        ...
        case CE_ItemViewItem :
        {
            if ( const QStyleOptionViewItem* vopt = qstyleoption_cast<const QStyleOptionViewItem*>( option ) )
            {
                ...
                const bool isNavigationTreeView = itemTreeView && itemTreeView->property( NavigationViewStyleProperty ).toBool();
                ...
                {
                    int treeCol = tv->treePosition();
                    if ( vopt->index.column() == treeCol )
                    {
                        int sectionSize = tv->header()->sectionSize( treeCol );
                        // 如果宽度等于整列宽，说明没有树枝缩进(比如顶层无子节点的扁平列表)
                        // 此时作为常规列处理，由 CE 自己画完整的圆角
                        isDecorationColumn = ( vopt->rect.width() < sectionSize );
                    }
                    else
                    {
                        isDecorationColumn = false;
                ...
                if ( iconRect.isValid() )
                {
                    ...
                    if ( isNavigationTreeView && isSettingsSpinItem )
                    {
                        ...
                        const QRect squareRect(
                            iconRect.x() + ( iconRect.width() - side ) / 2, iconRect.y() + ( iconRect.height() - side ) / 2, side, side );
                        ...
                        if ( !iconCode.isEmpty() )
                        {
                            ...
                            const QPixmap glyphPixmap = navigationGlyphPixmapCached( iconCode, glyphSide, vopt->palette.text().color() );
                ...
                if ( isNavigationTreeView )
                {
                    const bool isIconMode = ( widget && widget->property( "navigationIconMode" ).toBool() )
...
#    endif
        ...
        case CT_SpinBox :
        {
            if ( const auto* spinBoxOpt = qstyleoption_cast<const QStyleOptionSpinBox*>( option ) )
            {
                ...
                const int margins     = 8;
                // const int buttonWidth = hasButtons ? qRound( QStyleHelper::dpiScaled(
                // 16, dpi ) ) : 0;
        ...
        case CT_TabBarTab :
        {
            ...
            if ( isPivotStyle )
            {
                ...
                const int pivotHeight =
                    pivotTextTopPadding + contentVisualHeight + pivotIndicatorTopGap + pivotIndicatorHeight + pivotIndicatorBottomPadding;
            ...
            if ( widget && widget->property( TabBarStyleProperty ).toInt() == TabBarStyle::Navigation )
            {
                if ( const auto* tab = qstyleoption_cast<const QStyleOptionTab*>( option ) )
                {
                    const bool isVerticalNavigation = tab->shape == QTabBar::RoundedWest || tab->shape == QTabBar::RoundedEast
                                                      || tab->shape == QTabBar::TriangularWest || tab->shape == QTabBar::TriangularEast;
                    if ( isVerticalNavigation )
                    {
                        constexpr int indicatorWidth       = 3;
                        ...
                        if ( const auto* tabBar = qobject_cast<const QTabBar*>( widget ) )
                        {
                            ...
                            const int contentWidth =
                                contentLeadingInset + cachedIconWidth + iconTextSpacing + cachedTextWidth + contentTrailingInset;
...
void FluentUI3Style::polish( QWidget* widget )
{
    ...
    {
        polishCalendarWidget( calendar );
    }
    if ( auto* treeView = qobject_cast<QTreeView*>( widget ); treeView && treeView->property( NavigationViewStyleProperty ).toBool() )
    {
    ...
    const bool isCalendarPopup = widget->objectName() == "qt_datetimedit_calendar";
    ...
    else if ( isMenu || isComboPopup || isCalendarPopup )
    {
        bool wasCreated      = widget->testAttribute( Qt::WA_WState_Created );
        bool layoutDirection = widget->testAttribute( Qt::WA_RightToLeft );
    ...
    if ( auto treeView = qobject_cast<QTreeView*>( widget ); treeView && treeView->isAnimated() )
    {
        ...
        auto triggerBranchAnimation = [ treeView ]( const QModelIndex& index, bool opening )
        {
            ...
            const QByteArray animKey = ( isNavView ? QByteArrayLiteral( "_q_nav_branch_anim_" ) : QByteArrayLiteral( "_q_branch_anim_" ) )
        ...
        QObject::connect( treeView,
                          &QTreeView::expanded,
                          ctx,
                          [ triggerBranchAnimation ]( const QModelIndex& index ) { triggerBranchAnimation( index, true ); } );
        QObject::connect( treeView,
                          &QTreeView::collapsed,
                          ctx,
                          [ triggerBranchAnimation ]( const QModelIndex& index ) { triggerBranchAnimation( index, false ); } );
    }
    ...
    if ( auto* buttonBox = qobject_cast<QDialogButtonBox*>( widget ) )
    {
        static const QList<QDialogButtonBox::StandardButton> kAccentStdBtns = {
            QDialogButtonBox::Ok,   QDialogButtonBox::Yes,   QDialogButtonBox::Save,  QDialogButtonBox::SaveAll,
            QDialogButtonBox::Open, QDialogButtonBox::Apply, QDialogButtonBox::Retry,
        };
...
//     if (auto tooBtn = qobject_cast<QToolButton*>(widget); tooBtn && (
//     widget->objectName() == "ScrollLeftButton" || widget->objectName()
//     == "ScrollRightButton" ) )
//     {
//         tooBtn->setAutoRaise( true );
//     }
...
//         }
//     }
//     else if ( const auto* scrollarea = qobject_cast<QAbstractScrollArea*>(
...
//         {
//             const auto vp                    = scrollarea->viewport();
//             const auto wasAutoFillBackground = vp->property(
//             "_q_original_autofill_background" ).toBool();
//             vp->setAutoFillBackground( wasAutoFillBackground );
//             vp->setProperty( "_q_original_autofill_background", QVariant() );
//             const auto origStyledBackground = vp->property(
//             "_q_original_styled_background" ).toBool(); vp->setAttribute(
//             Qt::WA_StyledBackground, origStyledBackground ); vp->setProperty(
//             "_q_original_styled_background", QVariant() );
//         }
//         if ( auto table = qobject_cast<QTableView*>( widget ) )
//         {
...
void FluentUI3Style::drawCheckBox( const QStyleOption* option, QPainter* painter, const QWidget* widget ) const
{
    ...
    if ( isOn )
    {
        ...
        // 0.5 ); clipRect.setWidth( clipWidth * clipRect.width() );
...
QBrush FluentUI3Style::controlFillBrush( const QStyleOption* option, ControlType controlType ) const
{
    ...
    // checked is the same for Control (Buttons) and Control Alt
...
bool FluentUI3Style::eventFilter( QObject* watched, QEvent* event )
{
    ...
    //  {
    //      if (auto proEvent = static_cast<QDynamicPropertyChangeEvent*>(event);
    //      proEvent->propertyName() == "_q_colorscheme")
    //      {
    //          colorSchemeIndex = qApp->property("_q_colorscheme").toInt();
    //          auto appPalette = qApp->palette();
    //          polish(appPalette);
    //          qApp->setPalette(appPalette);
    //          qDebug()<< "[FluentUI3Style] _q_colorscheme changed" <<
    //          colorSchemeIndex;
    //      }
    ...
    else if ( auto* popup = qobject_cast<QWidget*>( watched );
              popup && ( popup->objectName() == "qt_datetimedit_calendar" ) && ( event->type() == QEvent::Show ) )
    {
        if ( auto* anchor = popup->parentWidget() )
        {
            const QPoint above = anchor->mapToGlobal( QPoint( 0, 0 ) );
            const QPoint below = anchor->mapToGlobal( QPoint( 0, anchor->height() ) );
            const int shadow   = FlyoutShadowBorderWidth - 2;

            QRect geom = popup->geometry();
            // 水平位置：让实心卡片左边缘与 anchor 控件左边缘对齐（向左偏移 shadow）
            geom.moveLeft( above.x() - shadow );

            const bool opensAbove = geom.center().y() < ( above.y() + below.y() ) / 2;
            if ( opensAbove )
            {
                geom.moveBottom( above.y() - FlyoutPopupOffset + shadow );
            }
            else
            {
                geom.moveTop( below.y() + FlyoutPopupOffset - shadow );
            }
    ...
    else if ( auto* viewport = qobject_cast<QWidget*>( watched ) )
    {
        ...
        if ( treeView && treeView->viewport() == viewport && treeView->property( NavigationViewStyleProperty ).toBool()
...
void FluentUI3Style::drawFluentDial( const QStyleOptionSlider* dial, QPainter* painter, const QWidget* widget, int style ) const
{
    ...
    if ( drawValue )
    {
        ...
        painter->drawText(
            QRectF( center.x() - knobRadius, center.y() - knobRadius, knobRadius * 2, knobRadius * 2 ), Qt::AlignCenter, valueText );
    }
    ...
    if ( style != DialThumb )
    {
        ...
        QPointF p2( center.x() + ( indicatorStart + indicatorLen ) * qCos( radAngle ),
                    center.y() - ( indicatorStart + indicatorLen ) * qSin( radAngle ) );
        painter->drawLine( p1, p2 );
    }
    ...
    else if ( style == DialThumb )
    {
        ...
        // Draw the thumb inside the track ring - same style + animation as QSlider
        // handle
        ...
        if ( widget )
        {
            const_cast<QWidget*>( widget )->setProperty(
                "_q_dial_thumb_rect",
                QRectF( thumbCenter.x() - outerRadius, thumbCenter.y() - outerRadius, outerRadius * 2, outerRadius * 2 ) );
        }
        ...
        if ( transitionsEnabled() && dial->styleObject )
        {
            ...
            bool doTransition =
                ( ( ( simulatedState & State_Sunken ) != ( oldState & State_Sunken ) ) || ( isThumbHovered != oldThumbHovered ) )
